...
public class Tree
{
    ...
    {
	int leftHeight = p.getLeft () != null ? p.getLeft ().getHeight ():
	0;
	int rightHeight = p.getRight () != null ? p.getRight ().getHeight ():
	0;
    ...
    {
	return ((root.getLeft () == null ? 0:
	root.getLeft ().getHeight ()) - (root.getRight () == null ? 0:
	...
	if (p != null)
	{
		...
		if (ancestorTree.balanceFactor () == -2)
		{
		    ...
		    if (rTreeBalanceFactor == -1 || rTreeBalanceFactor == 0)
		    { //0 happens in delete case 7a
			if (ancestor == root)
			{
		...
		else if (ancestorTree.balanceFactor () == 2)
		{
		    ...
		    if (lTreeBalanceFactor == 1 || lTreeBalanceFactor == 0)
		    { // 0 this symmetrical case of 7a does not happen. it's here to make the method symmetric and possible future optimization
			if (ancestor == root)
			{
	...
	if (p != null)
	{
		...
		{ //at this point it must have two children, thus it won't crash with p.getLeft (if null)
		    if (p == root)
			root = q;
		    else
			setParentsChildLink (p, q);
    ...
    public void updateTreeHeightsAbove (TNode p)
    {
	if (p != null)
	{
	...
	if(start != null && end != null){
		...
		if(start.getId().compareTo(root.getId()) <= 0
	...
	TNode q;
	for(int i = 0; i < 99; i++){
	    p = new TNode(Utils.leftPad("" + i, 4, '0'), 0);
	    x.insertNode(p);
	}
